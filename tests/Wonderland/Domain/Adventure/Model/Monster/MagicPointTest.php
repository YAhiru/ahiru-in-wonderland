...
final class MagicPointTest extends TestCase
{
    /**
     * @test
    ...
    public function canConstructDataProvider() : array
    {
        ...
        $current = 0;
        $max = 100;
        $cases['現在MPが0'] = [$current, $max];

        // case2 -------------------------
        $current = 100;
        $max = 100;
        $cases['現在MPと最大MPが同じ'] = [$current, $max];

        // case3 -------------------------
        $current = 1;
        $max = 1;
        $cases['最大MPが1'] = [$current, $max];

        // case4 -------------------------
        $current = 100;
        $max = 999;
        $cases['最大MPが999'] = [$current, $max];
    ...
    public function canNotConstructDataProvider() : array
    {
        ...
        $cases['現在MPは0以上の値であること'] = [$current, $max, $exceptionMessage];

        // case2 -------------------------
        $current = 0;
        $max = 0;
        $exceptionMessage = '$max must be greater than 1. but given ' . $max;
        $cases['最大MPは1以上の値であること'] = [$current, $max, $exceptionMessage];

        // case3 -------------------------
        $current = 10;
        $max = 1001;
        $exceptionMessage = 'MagicPoint must be less than 1000. but given ' . $max;
        $cases['最大MPは1000未満の値であること'] = [$current, $max, $exceptionMessage];

        // case4 -------------------------
        $current = 101;
        $max = 100;
        $exceptionMessage = '$current must be less than or equal to $max. but given $current=' . $current . ' $max=' . $max;
        $cases['現在MPは最大MP以下であること'] = [$current, $max, $exceptionMessage];
